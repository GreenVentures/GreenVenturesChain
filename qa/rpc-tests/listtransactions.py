...
def run_test(nodes):
    # Simple send, 0 to 1:
    txid = nodes[0].sendtoaddress(nodes[1].getnewaddr(), 0.1)
    sync_mempools(nodes)
    ...
    # send-to-self:
    txid = nodes[0].sendtoaddress(nodes[0].getnewaddr(), 0.2)
    check_array_result(nodes[0].listtransactions(),
    ...
    send_to = { nodes[0].getnewaddr() : 0.11, nodes[1].getnewaddr() : 0.22,
