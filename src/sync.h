 ...
 */
template <typename PARENT>
class LOCKABLE StdAnnotatedMixin : public PARENT
{
public:
    ~StdAnnotatedMixin() {
...
#define STD_LOCK(cs) DebugLock<decltype(cs)> PASTE2(criticalblock, __COUNTER__)(cs, #cs, __FILE__, __LINE__)

#define STD_WAIT_LOCK(cs, name) DebugLock<decltype(cs)> name(cs, #cs, __FILE__, __LINE__)
