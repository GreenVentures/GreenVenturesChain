...
void SplitHostPort(string in, int& portOut, string& hostOut) {
    ...
    bool fBracketed =
        fHaveColon &&
        (in[0] == '[' &&
         in[colon - 1] == ']');  // if there is a colon, and in[0]=='[', colon is not 0, so in[colon-1] is safe
...
bool Lookup(const char* pszName, vector<CService>& vAddr, int portDefault, bool fAllowLookup,
...
bool SetProxy(enum Network net, CService addrProxy, int nSocksVersion) {
    ...
    if (nSocksVersion != 0 && nSocksVersion != 4 && nSocksVersion != 5) return false;
...
CNetAddr::CNetAddr(const struct in6_addr& ipv6Addr) { memcpy(ip, &ipv6Addr, 16); }
...
vector<unsigned char> CNetAddr::GetGroup() const {
    ...
    if (nBits > 0) vchRet.push_back(GetByte(15 - nStartByte) | ((1 << nBits) - 1));
...
string CService::ToStringPort() const { return strprintf("%u", port); }
    ...
    if (valid_cidr) {
        ...
        if (network.IsIPv4())
            ...
            strNetmask = strprintf(
                "%x:%x:%x:%x:%x:%x:%x:%x", netmask[0] << 8 | netmask[1], netmask[2] << 8 | netmask[3],
...
string NetworkErrorString(int32_t err) {
    ...
    if (FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK, nullptr,
                       err, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), buf, sizeof(buf), nullptr)) {
